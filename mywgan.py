...
class MyWGAN:
	...
	def train(self, data, epoch, print_term, print_samples=0):
		...
		for i in range(int(np.shape(data)[0]//minibatches_size)):
			...
			for j in range(n_critic):
				...
				discriminator_loss.appens(discriminator_model.train_on_batch([image_batch, noise], [positive_y, negative_y, dummy_y]))
			generator_loss.append(generator_model.train_on_batch(self.noise_generating_rule(self.batch_size, self.noise_size), positive_y))
